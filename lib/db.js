...
class Database {
    ...
    async createOrder(orderNo, productId, productName, quantity, amount, contact = '') {
        this.checkReady();
        // 联系方式先存到remark字段，避免contact字段schema缓存问题
        const { data, error } = await this.client
            .from('orders')
            .insert({
                order_no: orderNo,
